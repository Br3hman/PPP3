...
[10] Change the “declaration keyword” from let to #.
	That is, instead of writing let x = 2; you would write #x = 2;
...
Token Token_stream::get()
{
	...
	if (!std::cin) return Token(quit); // stream failed/EOF: treat as quit instead of looping forever
	switch (ch) {
	...
	default:
		if (isalpha(ch) || ch == '#') {
			std::string s;
			s += ch;
			while (std::cin.get(ch) && (isalpha(ch) || isdigit(ch) || ch == '#')) s += ch; // collect the rest of the word
