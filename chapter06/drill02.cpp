...
Token Token_stream::get()
{
	...
	default:
		if (isalpha(ch)) {
			...
			if (s == "quit") return Token(quit); // recognize the word "quit" as well as a bare 'Q'
			return Token(name, s); // anything else is a variable name
		}
...
struct Variable { // a name/value pair for a declared variable
	std::string name;
	double value;
	Variable(std::string n, double v) :name(n), value(v) { }
};

std::vector<Variable> names; // table of all declared variables

double get_value(std::string s) // look up the value of variable s
{
...
void set_value(std::string s, double d) // assign d to variable s
{
	for (int i = 0; i <= names.size(); ++i)
		if (names[i].name == s) {
			names[i].value = d;
...
bool is_declared(std::string s) // has variable s already been declared?
{
	for (int i = 0; i < names.size(); ++i)
...
// Primary: Number | Name | "(" Expression ")" | "-" Primary
double primary()
{
	Token t = ts.get();
	switch (t.kind) {
