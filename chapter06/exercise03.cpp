/*
	...
	Hint: You have to add a member to Variable that distinguishes between
	constants and variables and check for it in set_value(). If you want to
	let the user define constants (rather than just having pi and e defined
	as constants), you’ll have to add a notation to let the user express that,
	for example, const pi = 3.14;.

	updated:
	- Variable class now has a boolean flag to indicate if the variable is a constant or not.
	If it is a constant, then it cannot be assigned a new value after declaration.
	The assignment operator checks for this and throws an error if an attempt is
	made to assign a new value to a constant variable.
	- define_name() function now has an additional boolean parameter to indicate
	if the variable being defined is a constant or not. This allows for predefined
	constants like pi, e, and k to be defined as constants.
	- The grammar has been updated to include the "const" keyword for constant variable declarations.
	- The assignment operator has been added to allow for changing the value of a variable after it
	has been introduced using 'declkey'. This can be useful for updating the value of a
	variable during calculations, but it can also be a source of problems
	if not used carefully, especially with constant variables.
...
double declaration(bool is_constant = false)
{
	...
	std::string n = t.name;
	if (is_declared(n)) throw std::runtime_error(n+ " declared twice");
	Token t2 = ts.get();
	if (t2.kind != '=') throw std::runtime_error("= missing in declaration of "+ n);
	double d = expression();
	var_table.push_back(Variable(n, d, is_constant));
	return d;
}

double assignment(std::string name )
{
	if (!is_declared(name)) throw std::runtime_error(name+ " is not declared");
	double d = expression();
	set_value(name, d);
	return d;
}
