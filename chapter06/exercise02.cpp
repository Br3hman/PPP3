/*
	[2] Provide an assignment operator, =, so that you can change the value of
	a variable after you have introduced it using let. Discuss why that can be
...
double define_name(std::string var, double val){ // declare a variable directly, bypassing the parser (used for predefined names like pi/e/k)
    if(is_declared(var)){
